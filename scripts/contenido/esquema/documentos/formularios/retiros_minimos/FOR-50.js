 ...
 // document.getElementById('fechaImagen').value = fecha;
  ...
  if (tipoDocumento !== "" && numeroDocumento !== "" || complemento !== "" || fechaNacimiento !== "") {
      if (subClasificacion === ""){
          ...
          const tipo_busqueda = (subClasificacion === 'CVEAP-A10') ? 'NRF' : 'T';
...
//  document.addEventListener("DOMContentLoaded", function() {
  ...
  (function() {
        cargarEnteGestor();
        cargarValoresCuentaPrevicional();
  })();


    function cargarValoresCuentaPrevicional() {

let  _CASO_NOMBRE = document.getElementById("_CASO_NOMBRE").value;

              console.log("_CASO_NOMBRE >>>>>>>>>>>>>>", _CASO_NOMBRE);
              console.log("cargarValoresCuentaPrevicional >>>>>>>>>>>>>>");
              console.log("cargarValoresCuentaPrevicional >>>>>>>>>>>>>>");
              console.log("cargarValoresCuentaPrevicional >>>>>>>>>>>>>>");
              console.log("cargarValoresCuentaPrevicional >>>>>>>>>>>>>>");
 ...
 $.ajax({
            dataType: 'json',
            type: 'GET',
            url: 'https://sgg.test.gestora.bo/otorgamiento-prestaciones-retiros/api/v1/reportesRetiro/getValorCuota',
            success: function(valor) {
                ...
                $.ajax({
                    dataType: 'json',
                    type: 'GET',
                    url: 'https://sgg.test.gestora.bo/otorgamiento-prestaciones-retiros/api/v1/reportesRetiro/getCuotasCertificado?numero_tramite='+_CASO_NOMBRE,
                    success: function(data) {
            
                        console.log('valor del estado v2 ',data);

                        if(data.codigo =='200'){ 
                            const num2= data.data;
                            const numero_cotas = num2.replace(',', '.'); 
                            document.getElementById("AS_NUM_CUOTAS").value = numero_cotas;
                            document.getElementById("AS_NUM_CUOTAS").dispatchEvent(new Event('input'));
                            console.log("cargarValoresCuentaPrevicional >>>>>>>>>>>>>>", );
                            console.log("data.data >>>>>>>>>>>>>>",data.data);numero_cotas
                            const AS_SALDO_ACUMULADO = valorCuota * numero_cotas ; 
 ...
 document.getElementById("AS_SALDO_ACUMULADO").addEventListener("change", function() {
    ...
    const  numero_cotas =     document.getElementById("AS_SALDO_ACUMULADO").value;
    var overlay = document.getElementById("overlay");
    overlay.style.display = 'flex';
                            ...
                            var listaMensajes = `
                                ...
                                var modalTitulo = document.getElementById("modalGenerico-titulo");
                                var modalMensaje = document.getElementById("modalGenerico-mensaje");
                                modalTitulo.textContent = "Esta Seguro  del Saldo Acumulado en la Cuenta Personal Previsional en Bolivianos";
        ...
        var AS_SALDO_ACUMULADO = document.getElementById("AS_SALDO_ACUMULADO").value;
        var AS_VALOR_CUOTA = document.getElementById("AS_VALOR_CUOTA").value;
        var AS_SALDO_ACUMULADO =   AS_SALDO_ACUMULADO/AS_VALOR_CUOTA;
  ...
  function retiroOpcion(){
      ...
      if ( a == 'RM' ) {
          _enable("RMI_REF_SALARIAL");
          _enable("RMI_SALARIO_MINIMO");
      } else {
          _disable("RMI_REF_SALARIAL");
          _disable("RMI_SALARIO_MINIMO");
      }
  }
