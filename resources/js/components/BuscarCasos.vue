    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <select v-model="filtro.prc_codigo" class="form-control" @change="listarRegistros">
                            <option value="">-- Todas --</option>
                            <option v-for="proceso in procesos" :value="proceso.prc_data.prc_codigo">
                                {{ proceso.prc_data.prc_codigo }} - {{ proceso.prc_data.prc_descripcion }}
                            </option>
                        </select>
                    </div>
                    /
                    ...
                    /
                    <div class="col-md-2">
                        <input type="text" v-model="filtro.cas_correlativo" class="form-control"
                            placeholder="Nro. Correlativo" @keyup.enter="onEnter">
                    </div>
                       <div class="col-md-2">
                        <select v-model="filtro.cas_gestion" class="form-control" @change="listarRegistros">
                            <option v-for="option in years" :key="option.year" :value="option.year">
                                {{ option.label }}
                            </option>
                        </select>
                    </div>
            ...
            <div class="card-body">
                <div class="container-fluid">
                    ...
                    <table class="table table-bordered table-striped background-container">
                        <thead class="thead-dark">
                            <tr>
                                  <th scope="col">Nro</th>
                                <th scope="col">
                                    #
                                    <button @click="sort('cas_id', 'asc')" class="btn btn-sm btn-outline-light">
                                        <i class="fas fa-arrow-up"></i>
                                    </button>
                                    <button @click="sort('cas_id', 'desc')" class="btn btn-sm btn-outline-light">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(r, index) in ordenarDatos" :key="r.cas_id">
                                ...
                                <td scope="row">
                                    ...
                                    {{ r.nodo_descripcion }}  -
                                        <span v-if="r.cas_estado === 'A'"
                                            class="badge badge-success"><strong>LI</strong></span>
                                        <span v-else-if="r.cas_estado === 'T'"
                                            class="badge badge-success"><strong>AS</strong></span>
                                        <span v-else-if="r.cas_estado === 'H'"
                                            class="badge badge-success"><strong>AR</strong>
                                            <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                                                <a class="dropdown-item">
                                                    <div class="media">
                                                        <div class="media-body">
                                                            <p class="text-sm text-muted"><i class="fa fa-angle-right"></i> <strong> Motivo de Archivo: </strong> </p>
                                                            <p class="text-sm"> {{ r.cas_data.cas_motivo_archivo }}
                                                            </p>
                                                            <hr>
                                                            </hr>
                                                            <p class="text-sm text-muted"><i
                                                                    class="fa fa-angle-right"></i> <strong>
                                                                    Descripción:</strong> </p>
                                                            <p class="text-sm"> {{
                                                                r.cas_data.cas_descripcion_archivo }} </p>
                                                        </div>
                                                    </div>
                                                </a>
                                            </div>
                                        </span>
                                        <span v-else-if="r.cas_estado === 'E'"
                                            class="badge badge-success"><strong>E</strong></span>
                                        <span v-else-if="r.cas_estado === 'W'"
                                            class="badge badge-success"><strong>W</strong></span>
                                ...
                                <td>
                                    ...
                                    <template v-if="r.cas_data.AS_TIPO_EAP">
                                        <span class="badge"
                                            style="background: linear-gradient(45deg, #EDCD4E, #f2dd9b); font-size: 0.8em;">
                                ...
                                <td align="center">
                                    ...
                                    <template v-if="mostrarEstadoGeneracionEAP(r.cas_data_valores)">
                                        <span class="badge"
                                            ...
                                            <strong
                                                v-if="mostrarEstadoGeneracionEAP(r.cas_data_valores) === 'M'">MANUAL</strong>
                                            <strong v-else-if="mostrarEstadoGeneracionEAP(r.cas_data_valores) === 'A'">
                                                AUTOMATICO </strong>
                                ...
                                <td>
                                    <i class="fas fa-calendar" style="color:#274690"></i> {{ r.cas_registrado.substr(0,
                                    10) }} <br> <i class="fas fa-clock" style="color:#274690"></i> {{
                                        r.cas_registrado.substr(10, 5) }} <br>
                                    <hr>
                                    <i class="fas fa-calendar" style="color:#297373"></i> {{ r.cas_modificado ?
                                        r.cas_modificado.substr(0, 10) : '-' }}
                                    <br> <i class="fas fa-clock" style="color:#297373"></i> {{ r.cas_modificado ?
                                        r.cas_modificado.substr(10, 5) : '-'
                                    }}
                                </td>
                                <td align="center">
                                ...
                                <td>
                                    <div style="display: inline-block;">
                                        <button type="button" class="btn btn-primary btn-circle" title="Histórico"
                                            v-on:click="doHistorico(r.cas_id, r.cas_padre_id, r.cas_cod_id)" data-toggle="modal"
                                        ...
                                        <template v-if="r.cas_data.ORIGEN && r.cas_data.ORIGEN === 'OV' && r.cas_cod_id.split('/')[0] === 'JUB1582' && r.act_data.act_orden === '20'">
                                        <!-- {{ r.cas_data.ORIGEN }} 0 -->
                                            <div v-for="nodo in usrNodos" style="display: inline-block;">
                                                <button v-if="r.cas_estado === 'T' && nodo.nodo_codigo === r.nodo_codigo"
                                                    v-on:click="doTomarCasoOV(r.cas_id, r.cas_data.AS_CUA)" type="button"
                                                    class="btn btn-warning btn-circle" title="Obtener OV" style="background-color: orange; border-color: orange;">
                                                    <i class="fas fa-hands" aria-hidden="true"></i>
                                                </button>
                                            </div>
                                        </template>

                                        <template v-if="r.act_data.act_orden !== '200' && (r.cas_cod_id.split('/')[0] !== 'JUB1582' || r.cas_data.ORIGEN === 'TRAMITESIP' || r.cas_data.ORIGEN === 'OV')">
                                        <!-- {{ r.cas_data.ORIGEN }} 2 -->
                                            <div v-for="nodo in usrNodos" style="display: inline-block;">
                                                <button v-if="r.cas_estado === 'T' && nodo.nodo_codigo === r.nodo_codigo"
                                                    v-on:click="doTomarCaso(r.cas_id, r.cas_padre_id, r)" type="button"
                                                    class="btn btn-warning btn-circle" title="Obtener TRAMITESIP">
                                                    <i class="fas fa-hands" aria-hidden="true"></i>
                                                </button>
                                            </div>
                                        </template>

                                        <template v-if="r.estado === 'H' || r.prc_data.prc_codigo === 'ARCH'">
                                            <button type="button" class="btn btn-success btn-circle"
                                                title="Complementar Documentos"
                                                v-on:click="doDocumentoPdfEdit(r.cas_id)" data-toggle="modal"
                                                data-target="#modalDocumentoPdfEdit">
                                                <i class="fa fa-list-ol" aria-hidden="true"></i>
                                            </button>
                                        </template>
                                        <button v-if="r.cas_estado === 'W'" type="button"
                                            class="btn btn-info btn-circle" title="Tomar"
                                            v-on:click="cadadito(r.cas_id, r.cas_padre_id)" data-toggle="modal"
                                            data-target="#modalCandadito">
                                            <i class="fas fa-hands" aria-hidden="true"></i>
                                        </button>
                                        <button v-if="r.cas_estado !== 'H' && r.prc_data.prc_codigo !== 'INV' && r.prc_data.prc_codigo !== 'PM'
                                            && es_supervisor === true && r.cas_data.ESTADO_DERIVACION !== 'ANULADO' && r.cas_data.ESTADO_DERIVACION !== 'DESISTIDO'
                                            && r.act_data.act_orden !== '20' && r.act_data.act_orden !== '30'" type="button"
                                            class="btn btn-danger btn-circle" title="Anular / Desistir Caso"
                                            data-toggle="modal"
                                            v-on:click="recuperarDatosParaAnular(r.cas_id, r.cas_cod_id, r.cas_estado)"
                                            data-target="#modalAnularDesistir">
                                ...
                                <td>
                                    <!-- boton de consultas -->
                                    <div style="display: flex;" v-if="r.act_data.act_orden == '40' || r.historico_chat_ucpp == 1">
                                        <button
                                            data-toggle="modal"
                                            @click="() => { openChatModal(r.cas_cod_id, r.act_data.act_orden) }"
                                    ...
                                    <div style="display: flex;">
                                        <button
                                            v-if="(es_supervisor || es_atc) && (filtro.prc_codigo === 'INV' || filtro.prc_codigo === 'PM' || r.cas_cod_id.split('/')[0] === 'PM' || r.cas_cod_id.split('/')[0] === 'INV') && r.cas_estado !== 'H' && r.act_data.act_orden !== '200'"
                                            type="button" class="btn btn-success btn-circle" title="Adjuntar"
                                            data-toggle="modal" @click="agarrar(r.cas_id, r.cas_cod_id)"
                                    ...
                                    <div style="display: flex;">
                                        <button v-if="(es_supervisor || es_atc) && (filtro.prc_codigo === 'INV' || filtro.prc_codigo === 'PM' || r.cas_cod_id.split('/')[0] === 'PM' || r.cas_cod_id.split('/')[0] === 'INV')
                                            && r.cas_estado !== 'H' && r.act_data.act_orden !== '200'  && r.mensaje_rechazo_desistido && r.mensaje_rechazo_desistido.trim() !== '' " type="button" class="btn btn-warning btn-circle"
                                            title="Mensaje" data-toggle="modal"
                                            @click="abrirModalMensaje(r.mensaje_rechazo_desistido)" data-target="#modalMensaje">
                                            <i class="fas fa-envelope" aria-hidden="true"></i>
                                        </button>
                                    </div>
                                </td>
                                <td>
                                    <span v-html="r.cas_data.cas_departamento"></span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
export default {
    ...
    methods: {
        ...
        bRegistros() {
            axios.get(`api/buscarCasos1/?search=${this.buscarRegistro}`)
                .then(response => {
        ...
        listarRegistros() {
            ...
            let url = "api/buscarCasos/" + this.RegistrosXPagina + '/' + this.PaginaActual + '?usuario=' + this.usrUser + '@gestora.bo&pro=buscar_caso_boton_buscar&tramite=tramites';
        ...
        doHistorico(id, id_padre, tramite) {
            ...
            this.casIdForLegalFlow = id_caso;
            this.casCodId = tramite;
            let that = this;
